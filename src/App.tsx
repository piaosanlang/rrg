...
export default function RRGChart() {
  ...
  // ── View state ────────────────────────────────────────────────
  const [view, setView]               = useState<"sectors" | "drilldown">("sectors");
  ...
  const [axisRange, setAxisRange] = useState({
    minRS: 93, maxRS: 108, minMom: 93, maxMom: 108,
  });

  // Active dataset — switches between sector and drilldown view
  const data = view === "drilldown" ? drilldownData : sectorData;

  const toX = useCallback(
    (rs: number) =>
      PAD.left + ((rs - axisRange.minRS) / (axisRange.maxRS - axisRange.minRS)) * CW,
    [axisRange, CW]
  );
  ...
  // ── Axis range helper ─────────────────────────────────────────
  const computeAxisRange = useCallback((dataset: any[]) => {
    const allRS  = dataset.flatMap((d: any) => d.trail.map((p: any) => p.rs));
    const allMom = dataset.flatMap((d: any) => d.trail.map((p: any) => p.mom));
    const pad = 2.0;
    return {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}rrg-data.json`)
      ...
      .then((json) => {
        ...
        const tailLen = json[0]?.trail?.length ?? TAIL_LENGTH;
        setAnimFrame(tailLen - 1);
        frameRef.current = tailLen - 1;
      })
      .catch((err) => console.error("Failed to load rrg-data.json.", err));
  }, [computeAxisRange]);

  // ── Drill-down fetch ──────────────────────────────────────────
  const handleDrilldown = useCallback((ticker: string) => {
    setDrilldownLoading(true);
    fetch(`${import.meta.env.BASE_URL}rrg-${ticker.toLowerCase()}.json`)
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json();
      })
      ...
      .catch((err) => console.error(`Failed to load drilldown for ${ticker}`, err))
  ...
  const handleBack = useCallback(() => {
    ...
    setPlaying(false);
    if (sectorData) {
      setAxisRange(computeAxisRange(sectorData));
      const tailLen = sectorData[0]?.trail?.length ?? TAIL_LENGTH;
      setAnimFrame(tailLen - 1);
      frameRef.current = tailLen - 1;
    }
  }, [sectorData, computeAxisRange]);
  ...
  useEffect(() => {
    ...
    data.forEach((d, i) => {
      ...
      // Trail segments
      for (let t = 0; t < n - 1; t++) {
        const p0 = visTrail[t], p1 = visTrail[t + 1];
        ...
        ctx.fillStyle = d.color + Math.round((0.15 + progress * 0.55) * 255).toString(16).padStart(2, "0");
        ctx.fill();
      }

      // Arrowhead
      if (n >= 2) {
        const p0 = visTrail[n - 2], p1 = visTrail[n - 1];
        drawArrowhead(ctx, toX(p0.rs), toY(p0.mom), toX(p1.rs), toY(p1.mom), isActive ? 10 : 7, d.color + "dd");
      }

      // Dot + glow
      const cur = visTrail[n - 1];
      const x = toX(cur.rs), y = toY(cur.mom), r = isActive ? 11 : 8;
      ...
      ctx.beginPath(); ctx.arc(x, y, r * 3, 0, Math.PI * 2); ctx.fillStyle = grd; ctx.fill();
    ...
    if (tooltipIdx !== null && data[tooltipIdx]) {
      ...
      if (fi >= 1) {
        ...
        dirText = `RS ${dr > 0.05 ? "↑" : dr < -0.05 ? "↓" : "→"}  MOM ${dm > 0.05 ? "↑" : dm < -0.05 ? "↓" : "→"}`;
      }
  ...
  }, [data, hovered, selected, animFrame, axisRange, toX, toY, CW, CH, view]);
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (canvasRef.current && selected.length > 0 && closest !== null && selected.includes(closest) && view === "sectors") {
        ...
        const dirText = animFrame >= 1;
        const bh = 82 + (dirText ? 18 : 0) + 20;
        const bw = 190;
        const bx = Math.min(x + 14, W - bw - 10);
        const by = Math.max(Math.min(y - 14, H - bh - 10), PAD.top);
        const linkY = by + bh - 12;

        if (mx >= bx + 8 && mx <= bx + bw - 8 && my >= linkY - 12 && my <= linkY + 4) {
    ...
    [data, animFrame, toX, toY, selected, view, W]
  );
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (selected.length > 0 && view === "sectors") {
        ...
        if (mx >= bx + 8 && mx <= bx + bw - 8 && my >= linkY - 12 && my <= linkY + 4) {
          handleDrilldown(d.ticker);
          return; // Navigate — do not toggle dot selection
        }
      }

      // Normal dot selection (pin/unpin tooltip)
      let closest: number | null = null;
      let minD = 22;
  ...
  const benchmarkLabel = view === "drilldown" ? activeSector! : BENCHMARK;
  const chartTitle     = view === "drilldown"
    ? `${activeSector} HOLDINGS RRG`
      ...
      <div style={{ width: "100%", maxWidth: 760, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {/* Back button — only in drill-down view */}
            {view === "drilldown" && (
              <button
                onClick={handleBack}
                style={{
            ...
            <div>
              ...
              <div style={{
                ...
                BENCHMARK: {benchmarkLabel} · WEEKLY · PYTHON-COMPUTED JDK-STYLE APPROXIMATION
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
            {Object.entries(quadCounts).map(([q, c]) => (
              <div key={q} style={{
                ...
                border: `1px solid ${QUADRANT_COLORS[q as keyof typeof QUADRANT_COLORS]}55`,
                borderRadius: 4,
                padding: "4px 12px",
                fontSize: 11,
